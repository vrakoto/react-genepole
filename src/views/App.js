...
function App() {
    ...
    // meme méthode que bougeXY mais bouge uniquement dans la plaque mixeur
    const bougerZoneMixante = () => {
        const colonnes = document.querySelectorAll('.colonne')
        if (selectedGoutte.color !== '' && selectedGoutte.color !== undefined) {
            try {
                const currentPosition = colonnes[selectedGoutte.idColonne - 1].children[selectedGoutte.idCellule - 1]
                ...
                const newPosition = colonnes[1].children[8]
                newPosition.classList.add('item')
                newPosition.style.backgroundColor = selectedGoutte.color
    ...
    /* const test = (currentPosition, newPosition) => {
        const colonnes = document.querySelectorAll('.colonne')
        const currentPosition = colonnes[selectedGoutte.idColonne - 1].children[selectedGoutte.idCellule - 1]
        currentPosition.classList.remove('item')
        currentPosition.style.backgroundColor = ''
            ...
            plaques_mixeur.forEach((plaque, key) => {
                if (plaque.classList.contains('item')) {
                    ...
                    switch (plaque.style.backgroundColor) {
                        case 'green': couleurPlaque = "#FF7700"; break;
                        case 'blue': couleurPlaque = "#804000"; break;
                        case 'yellow': couleurPlaque = "#800080"; break;
                        case 'red': couleurPlaque = "#ff0000"; break;
                    }
                    setTimeout(() => {
                        plaque.style.backgroundColor = couleurPlaque
                    }, temperatureVal.current.value * 1000);
                }
            })
        }
    }, [updateCellule])
            ...
            <div className="split left">
                ...
                {!openModal ? (
                    ...
                    <>
                        ...
                        {enableMixeur ? (
                            <button className="btn btn-warning ms-2" onClick={bougerZoneMixante}>Déplacer dans zone mixeur</button>
                        ) : ''}
                ...
                {enableMixeur ? (
                    <input type="number" ref={temperatureVal} placeholder="Température" />
                ) : ''}
